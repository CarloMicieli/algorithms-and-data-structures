...
private[this]
case class ListQueue[+A](front: List[A], back: List[A]) extends Queue[A] {

  def this() = this(List.empty[A], List.empty[A])

  override def enqueue[A1 >: A](el: A1): Queue[A1] = ListQueue(front, el +: back)
  ...
  override def peek: Maybe[A] =
    if (isEmpty)
      ...
      front match {
        case x +: _ => Just(x)
        case _      => back.reverse.headOption
      }
    }
  ...
  override def dequeue: Or[(A, Queue[A]), EmptyQueueException] =
    if (isEmpty)
      ...
      front match {
        case x +: xs => Good((x, ListQueue(xs, back)))
        case _ =>
          val newFront = back.reverse
          Good((newFront.head, ListQueue(newFront.tail, List.empty[A])))
      }
    }

  override def size: Int = front.length + back.length
