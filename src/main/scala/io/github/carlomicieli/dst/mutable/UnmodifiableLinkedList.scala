...
class UnmodifiableLinkedList[A](inner: LinkedList[A]) extends LinkedList[A] {
  ...
  override def headOption: Maybe[A] = inner.headOption

  override def lastOption: Maybe[A] = inner.lastOption
  ...
  override def find(p: (A) => Boolean): Maybe[A] = inner.find(p)
  ...
  override def removeHead(): Or[A, EmptyLinkedListException] = invalidOp
