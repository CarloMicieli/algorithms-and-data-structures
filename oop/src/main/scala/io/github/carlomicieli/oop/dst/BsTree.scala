...
class BsTree[K, V] extends Tree[K, V] {
  ...
  override def delete(key: K)(implicit ord: Ordering[K]): Option[V] = {
    findNode(key) match {
      case NIL => None
      case z =>
        treeDelete(z)
  ...
  override def search(key: K)(implicit ord: Ordering[K]): Option[V] = {
    findNode(key).toOption.map { p => p._2 }
  }
  ...
  sealed trait Node {
    ...
    def toOption: Option[(K, V)] = if (isNIL) None else Some(toPair)
